...
const ListTimerTask = () => {
  ...
  const handleCheckedSubmit = async (title, focus, check, myId) => {
    try {
      const req = await axios.patch("https://productivity-app-api-production.up.railway.app/TaskCurrent/" + myId, { task_title: title, focus_amount: focus, is_checked: check });
  ...
  const handleCurrentTask = async (title, taskId) => {
    const id = 1;
    try {
      await axios.patch("https://productivity-app-api-production.up.railway.app/UserSettings/CurrentTask/" + id, { current_task: title, task_id: taskId })
  ...
  //   const id = 1
  //   try {
  //     await axios.patch("https://productivity-app-api-production.up.railway.app/UserSettings/CurrentTask/" + id, { current_task: title })
  //   } catch (err) {
  //     console.log(err);
  //   }
  // }
